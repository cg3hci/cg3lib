 ...
 * namespace) you can easily use the helpers cg3::serializeObjectAttributes() and
 * cg3::deserializeObjectsAttributes();
 */
...
public:
     ...
     * - save the initial position of the input ifstream (see cg3::getFilePosition());
     * - inside a try block read all the data of the class;
     * - if an exception is catched, the initial position of the input ifstream should be restore  (see cg3::restoreFilePosition())
